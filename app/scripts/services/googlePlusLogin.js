...
angular.module('xbertsApp')
  ...
  this.$get = ['$rootScope', '$q', function ($rootScope, $q) {
    ...
    /**
     * Gets the current user id_token.
     * @returns {*} GoogleUser id_token string
     */
    NgGoogle.prototype.getUserId = function () {
      return auth2.currentUser.get().Zi.id_token;
    };
    ...
    function _initializeOnLoad() {
      ...
      auth2.currentUser.listen(function (user) {
        $rootScope.$broadcast('googlePlusLogin:currentUser', user);
        $rootScope.$apply();
      });

      auth2.isSignedIn.listen(function (isSignedIn) {
        $rootScope.$broadcast('googlePlusLogin:isSignedIn', isSignedIn);
        $rootScope.$apply();
      });
    }
